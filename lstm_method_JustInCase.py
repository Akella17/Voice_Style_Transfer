...
with tf.variable_scope("SpeakerRecognition") :
    ...
    seq_input = tf.placeholder(tf.float32, [None, batch_size, input_dim])
    Y_batch = tf.placeholder(tf.float32, [batch_size,output_dim])
    ...
    inputs = [tf.reshape(i, (batch_size, input_dim)) for i in tf.split(seq_input, seq_len, axis = 0)]
    ...
    cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=Y_batch, logits=dense_out))
...
#    batch_input = list(batch_input)
    feed = {seq_input: batch_input.astype('float32'), Y_batch: y_label.astype('float32')}
    #print(Y_batch.shape)
    ...
    #print(final_layer)
  print("-------------------------epoch : "+str(i))
  num_batches_test = X_val.shape[1]//batch_size - 1
  count = 0
  for batch in range(num_batches_test):
    batch_input = X_val[:, batch*batch_size:batch*batch_size+batch_size, :]
    y_label = Y_val[batch*batch_size:batch*batch_size+batch_size, :]
    y_pred = session.run(soft_prob, feed_dict={seq_input: batch_input.astype('float32')})
    y_pred_classes = np.argmax(y_pred, axis = 1)
    y_true = np.argmax(y_label, axis = 1)
    for it in range(len(y_pred_classes)):
      if(y_pred_classes[it] == y_true[it]):
        count+=1
